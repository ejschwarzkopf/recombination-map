...
for i in $(seq 1 1000); do
  ...
  for i in $(cat tmp1.hackready.txt); do echo ">$i" >> tmp1.fasta; fastahack -r $i ../../work_ref/matina_v1.1.fasta >> tmp.fasta; done
  ...
  grep "Total interspersed repeats" tmp.fasta.tbl | tr -s ' ' | cut -d " " -f 6 > tmp.IR.txt
